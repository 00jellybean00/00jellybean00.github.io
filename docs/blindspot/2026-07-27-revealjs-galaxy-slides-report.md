...
**2차 작업(2026-07-27 오후)에서 세 가지를 더했습니다.** 조명이 밝은 강의실에서는 어두운 화면이 잘 안 보입니다. 그래서 주소 끝에 `?light`를 붙이면 밝은 화면으로 바뀌게 했습니다. 그리고 파일을 올릴 때마다 검사가 자동으로 돌게 했습니다. 지금까지는 사람이 직접 돌려야 했고, 안 돌려도 그대로 배포됐습니다. 마지막으로 쓰이지 않는 글꼴 파일 40개를 지워 0.78메가바이트를 줄였습니다.
...
- `assets/galaxy.css:16-33` + `assets/galaxy-reveal.css` — `?light` 모드. `:root.light`가 팔레트 변수만 갈아끼우고, 별·성운은 끈다. **`pre`/`.figure`는 light에서 불투명 `#111827`로 고정** — 반투명이면 밝은 배경이 비쳐 회색이 되고 그 안의 차트/다이어그램(자바스크립트에서 다크 기준으로 색을 정함)이 안 보인다. 첫 시안이 정확히 이 결함이었다.
- `index.html` / `slides/demo/index.html` `<head>` — `?light` 감지는 인라인 스크립트. 외부 파일로 빼면 스타일시트보다 늦게 실행돼 어두운 화면이 한 프레임 스친다.
- `.github/check-links.js` — 배포에서만 깨지는 경로를 잡는다. 정적 `src`/`href` 외에 `loadScript()` 인자와 KaTeX `local:` 조립 경로까지 본다. 결함 6종 주입으로 검출력 확인. 주의: 검사 대상 복사본을 만들 때 참조 파일을 빠뜨리면 기준선이 실패하므로 결과가 무의미해진다(실제로 한 번 겪음).
...
- **mermaid 용량 최적화** — 3.4MB UMD 통짜. ESM 청크 분할은 하지 않음. 지연 로드로 초기 비용만 제거. **사용자가 "그대로 둠"으로 확정(2026-07-27)**.
- **덱 목록의 데이터화** — 랜딩의 덱 목록은 HTML 직접 기재. 덱이 늘면 분리.
- **`CLAUDE.md` 수정** — 요청 범위 밖이라 건드리지 않음. 프로젝트 규칙은 `slides/README.md`에 기록.
- **CI에서의 Jekyll 빌드** — Ruby 설치 비용 대비 이득이 작아 제외. `_config.yml` 제외 목록은 `check-links.js`가 문자열로 검사.
- **밝은 모드용 다이어그램·차트 색** — light에서도 상자를 어둡게 두는 방식으로 회피. 색을 두 벌 유지하지 않음.
- **화면상의 라이트 모드 토글 버튼** — 주소 파라미터만 제공. 발표 중 오조작 위험 때문.

이전 판단을 뒤집은 항목(2026-07-27 2차):
- ~~**KaTeX 폰트 포맷 정리** — 404 소음을 피하려 3종 유지~~ → **뒤집음.** 근거가 틀렸다. 20개 `@font-face` 전부 `woff2`가 첫 순위라 브라우저는 나머지를 아예 요청하지 않는다(실측: 요청된 글꼴 4개 전부 woff2, 실패 자원 0건). `.ttf`/`.woff` 40개 삭제, 0.78MB 절감.
- ~~**CI** — 검사는 사람이 수동 실행~~ → **뒤집음.** 사용자가 도입 선택. push/PR마다 문법·자체검사·링크검사 실행.
...
**사용자 확인 필요:** 없음. 이전 질문(mermaid 3.4MB)은 2026-07-27 "그대로 둠"으로 해소됐다.

2차 작업 이탈:
- 의도적 범위 제외였던 **CI**와 **KaTeX 폰트 정리**를 뒤집었다. 전자는 사용자 선택, 후자는 원래 근거가 사실과 달랐기 때문이다.
- unknowns 미해소 2번(밝은 강의실용 테마)의 재방문 시점을 앞당겨 지금 구현했다. 발표장 조명을 몰라도 주소 파라미터 방식이면 현장 대응이 되기 때문이다.
